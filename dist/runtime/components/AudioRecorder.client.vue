...
const props = defineProps({
  showResult: { type: Boolean, required: false, default: true },
  autoStart: { type: Boolean, required: false, default: false },
  logger: { type: Function, required: false, default: void 0 }
});
...
const {
  ...
  onRecordingStarted: (s) => {
    stream.value = s;
    emit("recording-started", s);
  },
  onRecordingStopped: (audioBlob, audioUrl2) => {
    emit("recording-stopped", audioBlob, audioUrl2);
  },
        ...
        <AnimatePresence>
            <motion.div v-if="audioUrl && props.showResult" key="playback-section"
                ...
                <motion.div class="text-center space-y-4" :initial="{ y: 20 }" :animate="{ y: 0 }"
                    ...
                    <a :href="audioUrl" :download="`recording-${(/* @__PURE__ */ new Date()).toISOString()}.webm`">
            ...
            <motion.div v-if="error" key="error-section" :initial="{ opacity: 0, x: -20, scale: 0.95 }"
                ...
                <motion.div class="flex items-start gap-3" :initial="{ y: 10 }" :animate="{ y: 0 }"
                    ...
                    <div class="flex-1">
                        ...
                        <motion.ul v-if="
  error.includes(t('audio-recorder.audio.errors.noMicrophoneFound')) || error.includes(t('audio-recorder.audio.errors.accessDenied'))
