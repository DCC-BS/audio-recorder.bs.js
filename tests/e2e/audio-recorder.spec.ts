...
test.describe('Audio Recorder', () => {
  test.beforeEach(async ({ page, context }) => {
    ...
    await page.addInitScript(() => {
      ...
      class MockMediaRecorder {
        ...
        private eventListeners: { [key: string]: Array<(event: Event) => void> } = {};
  ...
  test('should start and stop audio recording', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: /audio\.stopRecording/i })).toBeVisible();
    ...
    // Stop recording
    const stopButton = page.getByRole('button', { name: /audio\.stopRecording/i });
  ...
  test('should handle recording time formatting correctly', async ({ page }) => {
    ...
    // Stop recording
    await page.getByRole('button', { name: /audio\.stopRecording/i }).click();
  });
